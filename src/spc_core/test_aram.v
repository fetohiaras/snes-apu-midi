...
module test_aram #(
    // The simulation reference keeps its image in data/.  The Primer top
    // overrides this with the image stored in this project's src/data/.
    parameter MEM_INIT_FILE = "data/test_spc.spc.hex"
)(
    input clk,
    input [7:0] din, 
...
wire is_digit = din >= 8'h30 && din <= 8'h39;

initial begin
    $readmemh(MEM_INIT_FILE, mem);    // length will be default valid of 20 seconds
end
...
always @(posedge clk) begin
    ...
    if (write) begin
        ...
        else if ((addr == 17'haa || addr == 17'hab) && is_digit)
            int_length <= (int_length << 3) + (int_length << 1) + (din - 16'h30);
        else if (addr == 17'hac)
            int_fade <= {8'h0, din-8'h30};
        else if ((addr == 17'had || addr == 17'hae) && is_digit)
